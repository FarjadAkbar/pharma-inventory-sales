 ...
 * 
 * The monolithic HTTP server has been migrated to a microservices architecture.
 * The HTTP server is now in: backend/api-gateway/src/main.ts
 * 
 * This backend directory now contains only shared infrastructure code:
 * - Core entities and use cases
 * - Infrastructure (database, cache, logger, secrets)
 * - Libraries (token, event, i18n)
 * - Utils
 * 
 * All business logic modules have been extracted to microservices in:
 * - backend/services/*/
 

// This file is kept for reference but should not be executed.
// The HTTP server functionality has been moved to the API Gateway.
