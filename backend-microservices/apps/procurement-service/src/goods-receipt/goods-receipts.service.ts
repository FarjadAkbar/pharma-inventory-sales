...
export class GoodsReceiptsService {
  ...
  async create(createGoodsReceiptDto: CreateGoodsReceiptDto): Promise<GoodsReceiptResponseDto> {
    const purchaseOrder = await this.purchaseOrdersService.findOne(createGoodsReceiptDto.purchaseOrderId);
    if (!purchaseOrder) {
  ...
  private async toResponseDto(goodsReceipt: GoodsReceipt, items?: GoodsReceiptItem[]): Promise<GoodsReceiptResponseDto> {
    ...
    try {
      purchaseOrder = await this.purchaseOrdersService.findOne(goodsReceipt.purchaseOrderId);
    } catch {
      // ignore
    }
