...
import { CreateSiteDto, UpdateSiteDto, SiteResponseDto, SiteType } from '@repo/shared';
...
export class SitesService {
  ...
  async findAll(): Promise<SiteResponseDto[]> {
    const sites = await this.sitesRepository.find({ order: { createdAt: 'DESC' } });
    return sites.map((s) => this.toResponseDto(s));
  }
