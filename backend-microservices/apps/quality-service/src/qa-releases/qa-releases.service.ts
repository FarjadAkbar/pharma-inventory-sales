...
import { QCSamplesService } from '../qc-samples/qc-samples.service';
...
export class QAReleasesService {
  constructor(
    @InjectRepository(QARelease) private qaReleasesRepository: Repository<QARelease>,
    @InjectRepository(QAChecklistItem) private qaChecklistItemsRepository: Repository<QAChecklistItem>,
    private qcSamplesService: QCSamplesService,
    private qcResultsService: QCResultsService,
    @Inject('PROCUREMENT_SERVICE') private procurementClient: ClientProxy,
  ...
  async create(createQAReleaseDto: CreateQAReleaseDto): Promise<QAReleaseResponseDto> {
    const sample = await this.qcSamplesService.findOne(createQAReleaseDto.sampleId).catch(() => null);
    if (!sample) throw new NotFoundException('QC sample not found');
    ...
    for (const resultId of createQAReleaseDto.qcResultIds) {
      const result = await this.qcResultsService.findOne(resultId).catch(() => null);
      if (!result || !result.submittedToQA) throw new BadRequestException(`QC result ${resultId} must be submitted to QA`);
      if (result.sampleId !== createQAReleaseDto.sampleId) throw new BadRequestException(`QC result ${resultId} does not belong to sample ${createQAReleaseDto.sampleId}`);
    }
  ...
  async notifyWarehouse(id: number): Promise<QAReleaseResponseDto> {
    ...
    // Create putaway item in warehouse service
    try {
      await firstValueFrom(
        this.warehouseClient.send(WAREHOUSE_PATTERNS.PUTAWAY_CREATE, {
          ...
          unit: release.unit,
          qaReleaseId: release.id,
          goodsReceiptItemId: release.goodsReceiptItemId,
          requestedBy: release.submittedBy,
          remarks: `Auto-created from QA Release: ${release.releaseNumber}`,
        })
      );
    } catch (error) {
      console.error('Failed to create putaway item:', error);
      // Continue even if putaway creation fails - mark as notified anyway
    }
