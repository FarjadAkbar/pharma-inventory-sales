...
import { CreateQCTestDto, UpdateQCTestDto, QCTestResponseDto, QCTestStatus } from '@repo/shared';

@Injectable()
export class QCTestsService {
  constructor(
    @InjectRepository(QCTest) private qcTestsRepository: Repository<QCTest>,
    @InjectRepository(QCTestSpecification) private qcTestSpecificationsRepository: Repository<QCTestSpecification>,
    ...
    try {
      ...
      if (createQCTestDto.specifications && createQCTestDto.specifications.length > 0) {
        const specifications = createQCTestDto.specifications.map((spec) =>
          this.qcTestSpecificationsRepository.create({
            testId: savedTest.id,
  ...
  async findAll(): Promise<QCTestResponseDto[]> {
    ...
    return Promise.all(tests.map((t) => this.toResponseDto(t)));
  }

  async findOne(id: number): Promise<QCTestResponseDto> {
    const test = await this.qcTestsRepository.findOne({ where: { id }, relations: ['specifications'] });
    ...
    try {
      ...
      if (updateQCTestDto.specifications) {
        ...
        const specifications = updateQCTestDto.specifications.map((spec) =>
          queryRunner.manager.create(QCTestSpecification, {
            testId: id,
            parameter: spec.parameter,
            minValue: spec.minValue,
            maxValue: spec.maxValue,
      ...
      await queryRunner.commitTransaction();
      const updatedTest = await this.qcTestsRepository.findOne({ where: { id }, relations: ['specifications'] });
