...
export class QCSamplesService {
    ...
    if (createQCSampleDto.sourceType === 'GoodsReceipt') {
      ...
      try {
        goodsReceipt = await firstValueFrom(
          this.procurementClient.send<GoodsReceiptResponseDto>(GOODS_RECEIPT_PATTERNS.GET_BY_ID, createQCSampleDto.sourceId)
        );
      } catch (error) {
  ...
  async assignTests(id: number, assignTestsDto: AssignTestsToSampleDto): Promise<QCSampleResponseDto> {
    ...
    // Verify all tests exist using direct service call
    for (const testId of assignTestsDto.testIds) {
      try {
        await this.qcTestsService.findOne(testId);
      } catch (error) {
