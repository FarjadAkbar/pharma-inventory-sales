...
export class QCSamplesController {
  ...
  @MessagePattern(QC_RESULT_PATTERNS.SAMPLE_LIST)
  async sampleList(@Payload() params?: { status?: string }) {
    const samples = await this.qcSamplesService.findAll();
    if (params?.status) return { samples: samples.filter((s) => s.status === params.status) };
    return { samples };
  }
}
