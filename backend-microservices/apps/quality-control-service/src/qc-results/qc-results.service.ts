...
export class QCResultsService {
  constructor(
    ...
    private qcSamplesService: QCSamplesService, // Direct injection
    private qcTestsService: QCTestsService, // Direct injection
  ) {}

  async create(createQCResultDto: CreateQCResultDto): Promise<QCResultResponseDto> {
    // Verify sample exists using direct service call
    const sample = await this.qcSamplesService.findOne(createQCResultDto.sampleId);

    // Verify test exists using direct service call
    const test = await this.qcTestsService.findOne(createQCResultDto.testId);
  ...
  async submitToQA(submitResultsDto: SubmitResultsToQADto): Promise<QCResultResponseDto[]> {
    // Verify sample exists using direct service call
    const sample = await this.qcSamplesService.findOne(submitResultsDto.sampleId);
    ...
    // Update sample status using direct service call
    await this.qcSamplesService.update(submitResultsDto.sampleId, {
      status: 'Submitted to QA' as any,
    });
  ...
  async completeTesting(sampleId: number): Promise<void> {
    ...
    // Update sample status using direct service call
    await this.qcSamplesService.update(sampleId, {
      status: QCSampleStatus.QC_COMPLETE,
    });
  }
  ...
  private async toResponseDtoWithTest(result: QCResult): Promise<QCResultResponseDto> {
    let test: any = null;
    try {
      test = await this.qcTestsService.findOne(result.testId);
    } catch (error) {
