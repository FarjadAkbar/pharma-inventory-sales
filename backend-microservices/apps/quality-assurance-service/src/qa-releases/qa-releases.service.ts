...
export class QAReleasesService {
  constructor(
    ...
    @Inject('QUALITY_CONTROL_SERVICE') // Updated to use consolidated service
  ...
  async create(createQAReleaseDto: CreateQAReleaseDto): Promise<QAReleaseResponseDto> {
    ...
    try {
      sample = await firstValueFrom(
        this.qualityControlClient.send<QCSampleResponseDto>(QC_SAMPLE_PATTERNS.GET_BY_ID, createQAReleaseDto.sampleId)
      );
    } catch (error) {
    ...
    for (const resultId of createQAReleaseDto.qcResultIds) {
      try {
        const result = await firstValueFrom(
          this.qualityControlClient.send<QCResultResponseDto>(QC_RESULT_PATTERNS.GET_BY_ID, resultId)
        );
