...
export class QCSamplesController {
  ...
  async create(@Body() createDto: CreateQCSampleDto) {
    return await firstValueFrom(
      this.qcSampleClient.send(QC_SAMPLE_PATTERNS.CREATE, createDto)
    );
  }
  ...
  async findAll() {
    return await firstValueFrom(
      this.qcSampleClient.send(QC_SAMPLE_PATTERNS.LIST, {})
    );
  }
  ...
  async findByGoodsReceiptItem(@Param('goodsReceiptItemId') goodsReceiptItemId: string) {
    return await firstValueFrom(
      this.qcSampleClient.send(QC_SAMPLE_PATTERNS.GET_BY_GOODS_RECEIPT_ITEM, parseInt(goodsReceiptItemId, 10))
    );
  }
  ...
  async findOne(@Param('id') id: string) {
    return await firstValueFrom(
      this.qcSampleClient.send(QC_SAMPLE_PATTERNS.GET_BY_ID, parseInt(id, 10))
    );
  }
  ...
  async receiveSample(@Param('id') id: string) {
    return await firstValueFrom(
      this.qcSampleClient.send(QC_SAMPLE_PATTERNS.RECEIVE_SAMPLE, parseInt(id, 10))
    );
  }
  ...
  async assignTests(@Param('id') id: string, @Body() assignTestsDto: AssignTestsToSampleDto) {
    return await firstValueFrom(
      this.qcSampleClient.send(QC_SAMPLE_PATTERNS.ASSIGN_TESTS, {
        id: parseInt(id, 10),
        assignTestsDto,
      })
    );
  ...
  async delete(@Param('id') id: string) {
    await firstValueFrom(
      this.qcSampleClient.send(QC_SAMPLE_PATTERNS.DELETE, parseInt(id, 10))
    );
  }
}
